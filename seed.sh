...
echo ""
echo "=== 9. 商店與商品 ==="
SMS4_DB="$DB" $SMS4 shop open 1 "愛麗絲的小舖" --description "攝影器材、旅行用品、精選咖啡豆"
SMS4_DB="$DB" $SMS4 product add 1 "手沖咖啡壺組" 890 --stock 15 --description "Hario V60 經典手沖組，附濾紙 100 張"
SMS4_DB="$DB" $SMS4 product add 1 "衣索比亞 耶加雪菲 咖啡豆 250g" 450 --stock 20 --description "淺焙，柑橘花香，限量批次"
SMS4_DB="$DB" $SMS4 product add 1 "防水相機背帶" 350 --stock 30 --description "防滑設計，承重 5kg，旅行必備"
SMS4_DB="$DB" $SMS4 product add 1 "不鏽鋼保溫瓶 500ml" 680 --stock 25 --description "真空雙層，保溫 12 小時，登山露營適用"
SMS4_DB="$DB" $SMS4 product add 1 "輕量摺疊登山杖" 520 --stock 10 --description "碳纖維材質，僅重 180g，附收納袋"
SMS4_DB="$DB" $SMS4 shop open 2 "鮑勃的咖啡館" --description "自家烘焙咖啡豆與手沖器具"
SMS4_DB="$DB" $SMS4 product add 2 "瓜地馬拉 薇薇特南果 咖啡豆 250g" 380 --stock 30 --description "中焙，巧克力與堅果風味"
SMS4_DB="$DB" $SMS4 product add 2 "哥倫比亞 蕙蘭 咖啡豆 250g" 420 --stock 25 --description "中深焙，焦糖甜感，口感醇厚"
SMS4_DB="$DB" $SMS4 product add 2 "手沖濾杯 V60 01 陶瓷" 350 --stock 18 --description "日本原裝，簡約白"
SMS4_DB="$DB" $SMS4 shop open 3 "卡蘿的甜點廚房" --description "手工甜點與烘焙材料"
SMS4_DB="$DB" $SMS4 product add 3 "提拉米蘇 6 吋" 580 --stock 8 --description "經典義式，馬斯卡彭起司，每日限量"
SMS4_DB="$DB" $SMS4 product add 3 "手工焦糖布丁 4入" 320 --stock 15 --description "使用香草莢，濃郁滑順"
SMS4_DB="$DB" $SMS4 product add 3 "比利時巧克力布朗尼 6入" 280 --stock 20 --description "70% 黑巧克力，外酥內軟"
SMS4_DB="$DB" $SMS4 shop open 10 "傑克的旅行選物" --description "各國特色紀念品與旅行裝備"
SMS4_DB="$DB" $SMS4 product add 4 "京都清水燒 抹茶杯" 650 --stock 10 --description "手工繪製，每款圖案不同"
SMS4_DB="$DB" $SMS4 product add 4 "泰國手工香氛蠟燭" 320 --stock 20 --description "檸檬草與薄荷，舒緩放鬆"
SMS4_DB="$DB" $SMS4 product add 4 "旅行收納包 4件組" 480 --stock 30 --description "輕量防水材質，行李箱必備"
SMS4_DB="$DB" $SMS4 shop open 13 "小梅的手作工作室" --description "皮革製品與手作材料包"
SMS4_DB="$DB" $SMS4 product add 5 "真皮短夾 手工縫製" 1280 --stock 8 --description "義大利植鞣革，可刻字"
SMS4_DB="$DB" $SMS4 product add 5 "皮革鑰匙圈 DIY 材料包" 250 --stock 40 --description "附工具與說明書，新手友善"
